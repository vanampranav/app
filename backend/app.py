...
from rate_limit import enforce, require_api_key
...
async def user_endpoint(request: Request):
    try:
        ...
        if "userDetails" in data:
            ...
            food_preferences_pregen = str(data.get("foodPreferences", "")) or ""
        ...
        generation_data = {
            ...
            "weight": weight,
            "capped": capped,
            "foodPreferences": food_preferences_pregen,
        }
...
def _get_plan_day_mapping() -> dict:
    """
    Returns {1: 'Monday', 2: 'Tuesday', ...} for Day 1 (today) through Day 7,
    using IST (UTC+5:30) so the mapping is correct for Indian users regardless
    of what timezone the server runs in.
    """
    from datetime import datetime, timedelta, timezone
    ist_now = datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)
    day_names = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    return {i + 1: day_names[(ist_now + timedelta(days=i)).weekday()] for i in range(7)}


async def extract_user_preferences_llm(client, user_prompt: str, dietary_restrictions: list = None, macros: dict = None, food_preferences: str = None) -> dict:
    """
    CURATOR AGENT: Analyzes user request + macro requirements to generate HIGH-PROTEIN meal examples.
    Also processes food_preferences (usual foods the user eats) with frequency/goal-alignment logic.
    """
    ...
    food_prefs_section = ""
    if food_preferences and food_preferences.strip():
        day_map = _get_plan_day_mapping()
        day_calendar_lines = "\n".join(
            f"  Day {n} = {name}{' ← TODAY (plan starts here)' if n == 1 else ''}"
            for n, name in day_map.items()
        )
        # Reverse map: weekday name → Day N (for the curator to look up day names)
        name_to_day = {name: n for n, name in day_map.items()}

        food_prefs_section = f"""

USER'S USUAL FOODS (what they normally eat):
"{food_preferences}"

PLAN CALENDAR (Day 1 = today, plan runs for 7 consecutive days):
{day_calendar_lines}

CRITICAL: When the user says a day name like "Sunday", "Saturday", "Wednesday" etc.,
look it up in the PLAN CALENDAR above and use the correct Day N.
Example: if today is Wednesday, then "eat egg on Wednesday" → Day 1 (not Day 3).
If the named day does not appear in Day 1-7, skip it (it's outside this week's plan).

YOUR TASK: Analyse every food/drink mentioned and output a MANDATORY FOOD SCHEDULE.

STEP A — FREQUENCY MAPPING:
- "everyday" / "every day" / "daily" / "every morning" / "twice daily" → ALL 7 days
- "twice daily" / "2x a day" → include TWICE per day on all 7 days
- "on weekends" → whichever Day N corresponds to Saturday and Sunday in the PLAN CALENDAR above
- "on [day name]" / "every [day name]" → look up that day name in the PLAN CALENDAR above and use the matching Day N
- "3 times a week" / "thrice" → 3 spread days (e.g. Day1, Day3, Day5)
- "most days" / "usually" → 5 days (Day1-Day5)
- "sometimes" / "occasionally" → 2 days (Day2, Day5)
- No frequency word (e.g. "I eat idli for breakfast") → 2 days (Day1, Day4)
- If a day name the user mentions is NOT in the PLAN CALENDAR (outside this 7-day window) → skip it

STEP B — GOAL ALIGNMENT:
   TIER 1 (keep at full frequency): food aligns with the user's fitness goal
   TIER 2 (reduce frequency, note it): mildly suboptimal — keep but reduce
   TIER 3 (exclude, note it): directly conflicts with goal at any frequency

STEP C — OUTPUT FORMAT (copy this structure EXACTLY):

=== MANDATORY FOOD SCHEDULE ===
⚠️ THE MAIN MEAL PLANNER MUST FOLLOW THIS EXACTLY. VIOLATIONS = INVALID PLAN. ⚠️

DAILY ITEMS (must appear in ALL 7 days, every single day):
[List only foods that are daily. For beverages like tea/coffee include calorie info.]
[Format: "- [food] — [meal slot] — [N]x per day — list as '[food name] — 1 cup — [cal] kcal — [p]p/[f]f/[c]c'"]
[Tea with milk+sugar: ~50 kcal per cup, 2p/2f/7c]
[Plain tea: ~5 kcal per cup, 0p/0f/1c]
[If nothing is daily, write: NONE]

DAY-BY-DAY PROTEIN / MAIN FOOD ASSIGNMENT (ONE assigned meal per day, not all meals):
[Use the actual weekday name from the PLAN CALENDAR in parentheses, e.g. "Day 1 (Wednesday)"]
Day 1 ([actual weekday from PLAN CALENDAR]): [assigned food at specific meal slot, or "open — use a varied protein"]
Day 2 ([actual weekday from PLAN CALENDAR]): [assigned food at specific meal slot, or "open — use a varied protein"]
Day 3 ([actual weekday from PLAN CALENDAR]): [assigned food at specific meal slot, or "open — use a varied protein"]
Day 4 ([actual weekday from PLAN CALENDAR]): [assigned food at specific meal slot, or "open — use a varied protein"]
Day 5 ([actual weekday from PLAN CALENDAR]): [assigned food at specific meal slot, or "open — use a varied protein"]
Day 6 ([actual weekday from PLAN CALENDAR]): [assigned food at specific meal slot, or "open — use a varied protein"]
Day 7 ([actual weekday from PLAN CALENDAR]): [assigned food at specific meal slot, or "open — use a varied protein"]

HARD EXCLUSION RULES:
[For every day-specific food, write two rules:]
["- [food]: ONLY on Day N at [meal slot]. NEVER use it on other days."]
["- [food]: include it ONCE on Day N only (not at multiple meals on that day)."]
[If none, write: NONE]

GOAL NOTES (add to END-OF-PLAN-SUGGESTION only if Tier 2 or Tier 3 items exist):
[Format: "- [food]: [1 sentence user-friendly explanation]"]
[If none, write: NONE]

=== END MANDATORY FOOD SCHEDULE ===

IMPORTANT PRINCIPLES:
- User-preferred foods are the PRIMARY basis for the plan. Make it feel familiar.
- Only exclude/reduce for a clear, significant conflict with the goal.
- Nutritionally neutral or mildly suboptimal foods → keep them (Tier 1).
- Never be overly restrictive."""
    ...
    breakfast_protein = int(protein_target * 0.20)
    lunch_protein = int(protein_target * 0.35)
    snack_protein = int(protein_target * 0.10)
    dinner_protein = int(protein_target * 0.35)
...
- If food_preferences mentions specific foods (idli, roti, rice-dal, etc.) → infer cuisine from those
- If user mentions NO cuisine and no food preferences → default to WESTERN cuisine
...
Based on the detected cuisine, dietary type, and food preferences, output:
...
4. BREAKFAST IDEAS (~{breakfast_protein}g protein each):
   [List 4-5 breakfast-appropriate meals — PRIORITISE user's preferred breakfast foods if mentioned]
   [Must be actual breakfast foods - not lunch/dinner dishes]

5. LUNCH/DINNER IDEAS (~{lunch_protein}g protein each):
   [List 5-6 complete meals — PRIORITISE user's preferred lunch/dinner foods if mentioned]
   [Structure: ONE protein + ONE carb + vegetables]

6. SNACK IDEAS (~{snack_protein}g protein each):
   [List 3-4 light, protein-rich snacks appropriate for THIS cuisine]

7. ALLOWED CARBS (ONE per meal):
   [List carb options — include user's preferred carbs first]

8. MANDATORY FOOD SCHEDULE:
   [Only output this section if food_preferences was provided]
   [Follow STEP A/B/C above EXACTLY — output the full === MANDATORY FOOD SCHEDULE === block]
   [This section is passed verbatim to the main meal planner as a hard constraint]

CRITICAL RULES:
- Generate foods appropriate for the DETECTED cuisine only
- Every meal example must hit the protein target
- Do NOT default to any specific cuisine - detect from user's words or food preferences
...
- PRIORITISE user's familiar foods — this makes plans more sustainable
...
# ============================================================
# FOOD PREFERENCE EXCLUSION RULE EXTRACTOR
# Parses the curator's MANDATORY FOOD SCHEDULE and returns a
# system-prompt-level exclusion block (higher priority than user_message).
# ============================================================
def _extract_exclusion_rules(curator_directions: str) -> str:
    """
    Extracts HARD EXCLUSION RULES from the curator's MANDATORY FOOD SCHEDULE block
    and reformats them as an explicit system-prompt section.
    Returns empty string if no exclusion rules are found.
    """
    if not curator_directions or "HARD EXCLUSION RULES:" not in curator_directions:
        return ""
    try:
        start = curator_directions.index("HARD EXCLUSION RULES:") + len("HARD EXCLUSION RULES:")
        section = curator_directions[start:]
        # Find next double newline or end of MANDATORY FOOD SCHEDULE
        end_markers = ["GOAL NOTES", "=== END MANDATORY", "DAILY ITEMS", "DAY-BY-DAY"]
        end_pos = len(section)
        for marker in end_markers:
            idx = section.find(marker)
            if idx > 0 and idx < end_pos:
                end_pos = idx
        rules_text = section[:end_pos].strip()
        if not rules_text or rules_text.upper() == "NONE":
            return ""
        return f"""
=== ABSOLUTE PROTEIN EXCLUSION RULES (SYSTEM OVERRIDE — HIGHEST PRIORITY) ===
These rules come from the user's food schedule and OVERRIDE all variety/protein instructions:
{rules_text}
Any day that uses a food marked ONLY for a specific day is INVALID. Regenerate that day.
=== END EXCLUSION RULES ===
"""
    except Exception:
        return ""
...
async def generate_mealplan_stream(data: dict):
    ...
    mealplan_logger.info(f"[PREGEN_STREAM] Starting generation: {calories} kcal, dietary={dietary}, food_prefs={'yes' if food_preferences else 'no'}")
    ...
    # Get curator instructions (pass food_preferences for frequency/tier analysis)
    macros_with_calories = {**macros, 'calories': calories}
    user_preferences = await extract_user_preferences_llm(
        client, user_prompt, dietary, macros_with_calories, food_preferences or None)
    user_specific_directions = user_preferences.get("directions", "")

    # Extract hard exclusion rules from curator output to inject into system prompt
    exclusion_rules_for_system = _extract_exclusion_rules(user_specific_directions)
...
{dietary_rules}
{exclusion_rules_for_system}
=== FOOD SCHEDULE ENFORCEMENT ===
If the user message contains a === MANDATORY FOOD SCHEDULE === block:
- DAILY ITEMS: include on ALL 7 days. Beverages list with calorie values (e.g. "Chai - 1 cup - 50 kcal - 2p/2f/7c"). "2x per day" = list item twice per day.
- DAY-BY-DAY: follow each day's assigned food at the specified meal slot. Assigned food appears ONCE on that day only (e.g. chicken at dinner only, not also at breakfast/lunch).
- HARD EXCLUSION: food assigned to Day N must NEVER appear on other days. Also: if dal is assigned to Day 6 lunch, dal must NOT also appear at Day 6 dinner.
These constraints OVERRIDE variety instructions. A plan that violates them is INVALID.
...
async def meal_plan(request: Request):
    ...
    if food_preferences:
        mealplan_logger.info(f"[FOOD_PREFERENCES] Received: {food_preferences[:100]}...")
    ...
    # Pass macros with calories and food preferences for protein-aware curation
    macros_with_calories = {**macros, 'calories': calories}
    user_preferences = await extract_user_preferences_llm(
        client, user_prompt, dietary, macros_with_calories, food_preferences or None)
    user_specific_directions = user_preferences.get("directions", "")
...
=== MANDATORY FOOD SCHEDULE ENFORCEMENT (HIGHEST PRIORITY) ===

If the user message contains a === MANDATORY FOOD SCHEDULE === block:

1. DAILY ITEMS → Include on ALL 7 days at the specified frequency
   - Beverages (tea, coffee, etc.) → list with calorie values from the schedule
   - "2x per day" → list the item TWICE in the appropriate meal/snack slots each day
   - Example for tea: "Chai - 1 cup - 50 kcal - 2p/2f/7c" listed under snack (or as a beverage line)

2. DAY-BY-DAY PROTEIN ASSIGNMENT → The food assigned to each specific day is MANDATORY
   - If Day 3 says "Egg at breakfast" → Day 3 MUST have eggs as the main protein at breakfast ONLY
   - If Day 6 says "Dal + Rice at lunch" → Day 6's lunch MUST be dal with rice, and dal should NOT appear at dinner too
   - If Day 7 says "Chicken at dinner" → Day 7's dinner has chicken; breakfast and lunch use other proteins
   - "Open — use a varied protein" days → pick freely from curator's allowed proteins

3. HARD EXCLUSION RULES → A food assigned to one day MUST NOT appear on other days
   - If Chicken is ONLY Day 7 → chicken MUST NOT appear on Day 1-6, no exceptions
   - If Dal is ONLY Day 6 → dal MUST NOT appear on any other day
   - Assigned food appears ONCE on its day (at the specified meal slot), not at all meals of that day

These rules OVERRIDE the variety/rotation instructions below for the specific assigned foods.

=== VARIETY & ROTATION (for open days) ===
...
- Rotate through the curator's allowed proteins/dishes on OPEN days
- Different combinations each day: Day 1 fish+rice, Day 2 tofu+quinoa, Day 3 eggs+roti
...
async def analyze_meal(request: Request, image: UploadFile = File(...)):
    ...
    """
    # Flutter-only endpoint: require the shared API key + rate limit (paid OpenAI call).
    require_api_key(request)
    enforce(request, "analyze-meal")
