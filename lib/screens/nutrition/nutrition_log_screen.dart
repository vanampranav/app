...
class _NutritionLogScreenState extends State<NutritionLogScreen> {
  ...
  Future<void> _pickAndAnalyzeImage(MealType meal) async {
    ...
    // Show loading overlay using a key to safely dismiss it later
    ...
    try {
      debugPrint('MealVision: Calling analyzeMealImage...');
      final results = await _visionService.analyzeMealImage(imageFile);
      ...
      // Wait for the pop animation to finish before showing the next sheet
      ...
      // Convert results to MealEntries with safe parsing
      final List<MealEntry> initialEntries = [];
      for (var res in results) {
        try {
          // Robust key checking: LLMs sometimes vary keys (name vs foodName)
      ...
      debugPrint('MealVision: Showing confirmation sheet for ${initialEntries.length} items');
      
      // Show confirmation sheet
      final confirmedEntries = await showModalBottomSheet<List<MealEntry>>(
        context: context,
        isScrollControlled: true,
        useRootNavigator: true, // Use root navigator to ensure it's on top of everything
